import argparse, logging, os.path, json, uvicorn, copy
from time import time, strftime
from fastapi import FastAPI, WebSocket, UploadFile, File
from gevent.lock import BoundedSemaphore
from src.asr.frame_asr import FrameASR
...
asr = FrameASR(model_definition = {
               ...
               offset=4, asr_model=asr_model, cfg=cfg)
...
async def recognize(ws: WebSocket):
    ...
    while True:
        ...
        # print(data, 'aaaaaaaaaaaaaaaaaiiiiiiiiiiiiiiiiiiiii')
        data = data['text'] if 'text' in data else data
        data = data['bytes'] if 'bytes' in data else data
        # print(type(data), '0000000000000000000000000000000000000000000000000000000')
        # data = data if isinstance(data, bytes) else data
        ratio = 16000 / 8000
        # resampled_data = resampler.process(np.frombuffer(data, dtype="int16"), ratio)
        # print(type(resampled_data), '8887777777777777777777777')
        # print(np.frombuffer(data, dtype="int16"), 'aaaaaaaaaaaa8888888888888888888888888')
        # audio_16kHz = librosa.core.resample(np.frombuffer(data, dtype="int16"), 16000, 16000)
        if isinstance(data, bytes):
            ...
            text = asr.transcribe(resampled_data)
            print(text,'888888888888888888888888888888888')
            await ws.send_text(text)
        elif isinstance(data, str) and data == 'EOS':
            print('auyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy')
            eos_time = time()
            # text = stream.finishStream()

            if len(text):
                await ws.send_text(text)
                # print(text + 'asdasdasdas', end='')
                empty_counter = asr.offset
            # logger_Deepspeech.info("Recognized:" + text)
            # await ws.send_text('aaaaaoiooooooo')
            gSem.release()
            gSem_acquired = False
            start_time = None
        else:
